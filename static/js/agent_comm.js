...
function initWebSocket() {
    ...
    socket.on('agent_thinking', (data) => {
        ...
        if ((conversationInProgress || isViewVisible) && isMatchingSession) {
            ...
            console.log('Ignoring agent_thinking - not matching session or not in progress');
        }
    });
    ...
    socket.on('orchestration_message', (data) => {
        ...
        if ((conversationInProgress || isViewVisible) && isMatchingSession) {
            ...
            console.log('Adding message to history:', data.sender, data.message.substring(0, 50) + '...');
            addMessageToHistory(
                data.sender, 
                data.message, 
                data.timestamp, 
            ...
            console.log('Ignoring orchestration_message - not matching session or not in progress');
        }
    });
            ...
            agents.forEach(agent => {
                ...
                checkboxDiv.innerHTML = `
                    <input type="checkbox" 
                           ...
                           checked>
                    <label for="agent_${agent.name}" class="text-sm text-slate-700 cursor-pointer flex-1">
                        <span class="font-medium">${escapeHtml(agent.name)}</span>
                        ${agent.model ? `<span class="text-slate-500 text-xs">(${escapeHtml(agent.model)})</span>` : ''}
                    </label>
                `;
...
function getSelectedAgents() {
    const checkboxes = document.querySelectorAll('input[name="selectedAgents"]:checked');
    ...
    try {
        ...
        if (sessionsList) {
            if (sessions.length === 0) {
                sessionsList.innerHTML = '<p class="text-sm text-slate-500">No previous sessions found.</p>';
            } else {
                sessionsList.innerHTML = '';
                sessions.forEach(session => {
                    const sessionDiv = document.createElement('div');
                    sessionDiv.className = 'p-3 bg-slate-50 hover:bg-slate-100 rounded-lg border border-slate-200 cursor-pointer transition-colors';
                    sessionDiv.innerHTML = `
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <div class="font-medium text-slate-900">${escapeHtml(session.objective.substring(0, 60))}${session.objective.length > 60 ? '...' : ''}</div>
...
async function startNewSession() {
    ...
    try {
        ...
        if (data.success) {
            ...
            showNotification('Session started successfully - conversation beginning...', 'success');
...
async function resumeSessionWithTurns() {
    ...
    try {
        ...
        if (data.success) {
            showNotification('Session resumed successfully - conversation continuing...', 'success');
...
function addMessageToHistory(agentName, message, timestamp, respondingTo = null, respondingToMessage = null) {
    ...
    if (respondingTo) {
        ...
        if (respondingToMessage) {
            messagePreview = `: "${escapeHtml(respondingToMessage)}${respondingToMessage.length >= 200 ? '...' : ''}"`;
        }
        respondingToHtml = `<div class="text-xs text-slate-500 mb-2">↳ Responding to <span class="font-medium text-indigo-600">${escapeHtml(respondingTo)}</span>${messagePreview}</div>`;
    }
    
    messageDiv.innerHTML = `
        <div class="flex items-center justify-between mb-2">
            <div class="flex items-center gap-2">
                <strong class="text-indigo-600">${escapeHtml(agentName)}</strong>
            </div>
            <span class="text-xs text-slate-500">${timeStr}</span>
        </div>
        ${respondingToHtml}
        <div class="text-slate-700 whitespace-pre-wrap">${escapeHtml(message)}</div>
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cancel buttons
    document.getElementById('cancelNewSessionBtn').addEventListener('click', showSessionSelection);
    document.getElementById('cancelResumeBtn').addEventListener('click', showSessionSelection);
